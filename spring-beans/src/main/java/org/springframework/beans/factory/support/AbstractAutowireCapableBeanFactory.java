...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			// 诚然，其实 mbdToUse.prepareMethodOverrides() 代码块，并没有做什么实质性的工作，
		...
		try {//创建代理对象
			...
			// 其实在调用该方法之前 Spring 一直都没有创建 bean ，那么这里返回一个 bean 的代理类有什么作用呢？
			// AOP 的功能就是基于这个地方
		...
		try {//如果没有代理对象，就只能走常规的路线进行 bean 的创建了
			// 创建 Bean 对象
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			// BeanWrapper 是一个从 BeanDefinition 到 Bean 直接的中间产物
	 ...
	 *		1、Supplier 回调：#obtainFromSupplier(final String beanName, final RootBeanDefinition mbd) 方法。
	 *	    2、工厂方法初始化：#instantiateUsingFactoryMethod(String beanName, RootBeanDefinition mbd, @Nullable Object[] explicitArgs) 方法。
	 *	    3、构造函数自动注入初始化：#autowireConstructor(final String beanName, final RootBeanDefinition mbd, Constructor<?>[] chosenCtors, final Object[] explicitArgs) 方法。
	 *		4、默认构造函数注入：#instantiateBean(final String beanName, final RootBeanDefinition mbd) 方法。
	 *
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 判断缓存，如果缓存中存在，即已经解析过了，则直接使用已经解析了的
		...
		// 已经解析好了，直接注入即可
		if (resolved) {
			// autowire 自动注入，带有参数的构造方法，调用构造函数自动注入
		...
		// 参数时，又没获取到构造方法，则只能调用无参构造方法来创建实例了(兜底方法)
		return instantiateBean(beanName, mbd);
	}

	/**
	 * 使用 Supplier 接口完成 Bean 的初始化
	 *
	...
	protected BeanWrapper instantiateUsingFactoryMethod(
		...
		// 构造一个 ConstructorResolver 对象 , 是构造方法或者工厂类初始化 bean 的委托类
	...
	/**
	 * 获取该 bean 的非简单属性。什么叫做非简单属性呢？就是类型为对象类型的属性，
	 * 但是这里并不是将所有的对象类型都都会找到，比如 8 个原始类型，String 类型 ，
	 * Number类型、Date类型、URL类型、URI类型等都会被忽略
	 *
