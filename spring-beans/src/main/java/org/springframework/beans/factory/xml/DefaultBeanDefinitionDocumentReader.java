...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	/**
	 * @Description BeanDefinitionParserDelegate 是一个重要的类，它负责解析 BeanDefinition
	 * @Author yangsj
...
//		例：<bean id="studentService" class="org.springframework.core.StudentService" />
...
//		如果根节点非默认命名空间，执行自定义解析 例： <tx:annotation-driven>
		else {
	...
	/**
	 * @Description  如果根节点或子节点使用默认命名空间 则调用该方法执行默认解析
	 * @Author yangsj
...
//				存在
				...
				// 不存在
					...
					importCount = getReaderContext().getReader().loadBeanDefinitions(
							StringUtils.applyRelativePath(baseLocation, location), actualResources);/* 计算绝对路径 */
				}
				if (logger.isTraceEnabled()) {
