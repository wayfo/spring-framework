...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * @Description 内置的 ResourceLoader 资源定位器
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			else {//非 "classpath*:" 开头，且路径不包含通配符，直接委托给相应的 ResourceLoader 来实现。
	...
	/** 当 locationPattern 以 "classpath*:" 开头但是不包含通配符时，调用该方法加载资源
	 *  利用 ClassLoader 来加载指定路径下的资源,返回 classes 路径下和所有 jar 包中的所有相匹配的资源。
	 *
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		//遍历 URL 集合,将 URL 转换成 UrlResource
		while (resourceUrls.hasMoreElements()) {
			URL url = resourceUrls.nextElement();
			// #convertClassLoaderURL(URL url) 将 URL 转换成 UrlResource 对象。
